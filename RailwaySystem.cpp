...
void RailwaySystem::cancelTicket(std::string name, int tID)
{
    ...
    if(found)
    {
        ...
        while (temp!=NULL)
        {
            ...
            {
                bool freedFemaleSeat = (deletedPassenger.gender=='F'||deletedPassenger.gender=='f');
                if (freedFemaleSeat)
                {
                    temp->data.femaleSeats++;
                }
                ...
                if (queueCount > 0)
                {
                    ...
                    {
                        if (waitingQueue[i].trainID == tID)
                        {
                            bool waiterIsFemale = (waitingQueue[i].gender == 'F' || waitingQueue[i].gender == 'f');
                            if (freedFemaleSeat == waiterIsFemale)
                            {
                                waitIndex = i;
                                break;
                            }
                    ...
                    if (waitIndex != -1)
                    {
                        Passenger prioritizedP = waitingQueue[waitIndex];
                        for (int j = waitIndex; j < queueCount - 1; ++j)
                        {
                            waitingQueue[j] = waitingQueue[j + 1];
                        }
                        queueCount--;
                        std::cout << "Auto-reallocating seat to waiting list passenger: " << prioritizedP.name << " (Age: " << prioritizedP.age << ")\n";
                        bookTicket(prioritizedP);
                    }
                }
                return;
            }
